...
/**Eigenvalue decomposition of a real matrix.
     ...
     \ingroup Linalg
  */
RTensor eig_sym(const RTensor &A, const RTensor &B, RTensor *V) {
  ...
  RTensor auxA(A);
  double *a = tensor_pointer(auxA);
  RTensor auxB(B);
  double *b = tensor_pointer(auxB);
  ...
  dsygv(itype, jobz, uplo, n, a, n, b, n, w, &info);
#else
  blas::integer lwork = -1;
  {
    double work0{};
    F77NAME(dsygv)(&itype, &jobz, &uplo, &n, a, &n, b, &n, w, &work0, &lwork, &info);
    lwork = static_cast<blas::integer>(work0);
  }
  {
    SimpleVector<double> work(tensor::safe_size_t(lwork));
    F77NAME(dsygv)
	  (&itype, &jobz, &uplo, &n, a, &n, b, &n, w, work.begin(), &lwork, &info);
  }
#endif
