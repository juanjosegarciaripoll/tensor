...
namespace tensor_test {
  ...
  template<typename elt_t> elt_t _tan(elt_t x) { return tan(x); }
  template<typename elt_t> elt_t _sinh(elt_t x) { return sinh(x); }
  template<typename elt_t> elt_t _cosh(elt_t x) { return cosh(x); }
  template<typename elt_t> elt_t _tanh(elt_t x) { return tanh(x); }
  ...
  template<typename elt_t, typename elt_t2, elt_t2 f(elt_t), Tensor<elt_t2> fT(const Tensor<elt_t> &)>
  ...
  TEST(TensorUnaryOperatorTest, CTensorSinh) {
    test_over_tensors<cdouble>(test_unop<cdouble,cdouble,_sinh,sinh>, 6, 4, 30);
  }
  TEST(TensorUnaryOperatorTest, CTensorCosh) {
    test_over_tensors<cdouble>(test_unop<cdouble,cdouble,_cosh,cosh>, 6, 4, 30);
  }
  TEST(TensorUnaryOperatorTest, CTensorTanh) {
    test_over_tensors<cdouble>(test_unop<cdouble,cdouble,_tanh,tanh>, 6, 4, 30);
  }
