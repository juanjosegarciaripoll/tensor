...
AC_DEFUN([TENSOR_GTEST],[
  AC_CHECK_HEADER([gtest/gtest.h], gtest="yes", gtest="no")
  AM_CONDITIONAL([HAVE_GTEST], [test $gtest = yes])
  AM_CONDITIONAL([TENSOR_THREADSAFE_DEATHTEST],
                 [test "x${enable_threadsafe_deathtest}" == xyes])
])
