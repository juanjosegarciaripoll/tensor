...
TEST(RSparseTest, RSparseEmptyConstructor) { test_empty_constructor<double>(); }
...
void test_small_constructor() {
  {
    // sparse([1]);
    SCOPED_TRACE("1x1");
    Tensor<elt_t> T(igen << 1 << 1, gen<elt_t>(1.0));
    Sparse<elt_t> S(T);
    EXPECT_EQ(1, S.rows());
  ...
  }
  {
    // sparse([1, 0; 0, 2]);
    SCOPED_TRACE("2x2");
    Tensor<elt_t> T(igen << 2 << 2, gen<elt_t>(1.0) << 0.0 << 0.0 << 2.0);
    Sparse<elt_t> S(T);
    EXPECT_EQ(2, S.rows());
    EXPECT_EQ(2, S.columns());
  ...
  }
  {
    // sparse([1, 2; 0, 3]);
    SCOPED_TRACE("2x2");
    Tensor<elt_t> T(igen << 2 << 2, gen<elt_t>(1.0) << 0.0 << 2.0 << 3.0);
    Sparse<elt_t> S(T);
    EXPECT_EQ(2, S.rows());
    EXPECT_EQ(2, S.columns());
  ...
  }
  {
    // sparse([1, 0; 2, 3]);
    SCOPED_TRACE("2x2");
    Tensor<elt_t> T(igen << 2 << 2, gen<elt_t>(1.0) << 2.0 << 0.0 << 3.0);
    Sparse<elt_t> S(T);
    EXPECT_EQ(2, S.rows());
    EXPECT_EQ(2, S.columns());
  ...
  }
  {
    // sparse([1, 0, 4; 2, 3, 0]);
    SCOPED_TRACE("2x2");
    Tensor<elt_t> T(igen << 2 << 3, gen<elt_t>(1.0) << 2 << 0 << 3 << 4 << 0);
    Sparse<elt_t> S(T);
    EXPECT_EQ(2, S.rows());
...
TEST(RSparseTest, RSparseSmallConstructor) { test_small_constructor<double>(); }
...
TEST(RSparseTest, RSparseEyeSmall) { test_sparse_eye_small<double>(); }

TEST(CSparseTest, CSparseEyeSmall) { test_sparse_eye_small<cdouble>(); }
...
TEST(RSparseTest, RSparseRandomSmall) { test_sparse_random_small<double>(); }

TEST(CSparseTest, CSparseRandomSmall) { test_sparse_random_small<cdouble>(); }
