...
void test_small_constructor() {
  ...
  }
  {
    // sparse([1, 0; 0, 2]);
    SCOPED_TRACE("2x2");
	auto T = Tensor2D<elt_t>({{1.0, 0.0}, {0.0, 2.0}});
    Sparse<elt_t> S(T);
    EXPECT_EQ(2, S.rows());
    EXPECT_EQ(2, S.columns());
  ...
  }
  {
    // sparse([1, 2; 0, 3]);
    SCOPED_TRACE("2x2");
	auto T = Tensor2D<elt_t>({{1.0, 2.0}, {0.0, 3.0}});
    Sparse<elt_t> S(T);
    EXPECT_EQ(2, S.rows());
    EXPECT_EQ(2, S.columns());
  ...
  }
  {
    // sparse([1, 0; 2, 3]);
    SCOPED_TRACE("2x2");
	auto T = Tensor2D<elt_t>({{1.0, 0.0}, {2.0, 3.0}});
    Sparse<elt_t> S(T);
    EXPECT_EQ(2, S.rows());
    EXPECT_EQ(2, S.columns());
  ...
  }
  {
    // sparse([1, 0, 4; 2, 3, 0]);
    SCOPED_TRACE("2x2");
    auto T = Tensor2D<elt_t>({{1.0, 0.0, 4.0}, {2.0, 3.0, 0.0}});
    Sparse<elt_t> S(T);
    EXPECT_EQ(2, S.rows());
