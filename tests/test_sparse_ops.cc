...
namespace tensor_test {
  ...
  TEST(RSparseTest, BinopSmall) {
    ...
    // old code.
    {
      ...
      RSparse B(RTensor(igen << 1 << 1, rgen << 0.958326));
      EXPECT_TRUE(all_equal(full(A+B), full(A)+full(B)));
      EXPECT_TRUE(all_equal(full(A-B), full(A)-full(B)));
      EXPECT_TRUE(all_equal(full(A*B), full(A)*full(B)));
    }
    {
      RSparse A(RTensor(igen << 2 << 1, rgen << 0.958326 << 0.0));
  ...
  TEST(CSparseTest, BinopRandom) {
    test_over_fixed_rank_tensors<cdouble>(test_sparse_binop_random<cdouble>, 2, 7);
  }
